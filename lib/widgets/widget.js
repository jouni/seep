...
var Widget = Class.extend({
	...
	removeListener: function(id) {
		...
		if(!this.outBuffer.listeners)
			this.outBuffer.listeners = {}
		if(server) {
			// FIXME this is broken when updating the listeners dynamically at runtime (removing a server side listener, but still leaving some other server listeners)
			var hasServerListenersForTheType = false;
			for(var i=0; i < this.serverListeners.length; i++) {
				if(this.serverListeners[i].t == listener.t) {
					hasServerListenersForTheType = true;
					break;
				}
			}
			if(this.outBuffer.listeners.server && this.outBuffer.listeners.server[listener.t]) {
				this.outBuffer.listeners.server[listener.t] -= 1;
			} else if(!hasServerListenersForTheType) {
				if(!this.outBuffer.listeners.server)
					this.outBuffer.listeners.server = {};
