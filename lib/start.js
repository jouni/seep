...
program
    ...
    .action(function(file){
    	if(!file) {
    		logger.error("You must specify a file or a folder.")
    		return
    	}
		...
		if(path.extname(file) == ".js") {
			if(!seep.registerApp(file)) {
				logger.error("The file '"+file+"' doesn't export any Seep application prototype.")
			} else {
				fs.watchFile(file, {persistent: true, interval: DEPLOY_INTERVAL}, function(curr, prev) {
					if(curr.mtime.getTime() != prev.mtime.getTime()) {
						logger.info("Redeploying '" + file + "' ...")
						if(!seep.registerApp(file)) {
							logger.error("The file '"+file+"' doesn't export any Seep application prototype.")
						}
					}
			...
			watch_dir(file)
			var folder = file
			fs.watchFile(folder, {persistent: true, interval: DEPLOY_INTERVAL}, function(curr, prev) {
				if(curr.mtime.getTime() != prev.mtime.getTime()) {
					logger.info("Re-scanning deploy folder...")
					watch_dir(folder)
				}
			})
		}
...
function watch_dir(dir) {
	...
	for(var i=0; i < files.length; i++) {
	    if(path.extname(files[i]) == ".js") {
	    	...
	    	} else if(watched.indexOf(file) > -1) {
	    		if(!seep.registerApp(dir + "/" + file))
	    			logger.error("The file '"+file+"' doesn't export any Seep application prototype.")
	    	}
	    }
	}
